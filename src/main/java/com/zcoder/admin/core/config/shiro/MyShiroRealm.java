package com.zcoder.admin.core.config.shiro;
...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		// 到数据库查是否有此对象
		SysUser user = userDao.findByLoginName(loginName);// 实际项目中，这里可以根据实际情况做缓存，如果不做，Shiro自己也是有时间间隔机制，2分钟内不会重复执行该方法
		if (user != null) {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken)
		...
		// 查出是否有此用户
		SysUser user = userDao.findByLoginName(token.getUsername());
		if (user != null) {
